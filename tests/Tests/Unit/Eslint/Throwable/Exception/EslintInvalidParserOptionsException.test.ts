...
describe('EslintInvalidParserOptionsException', () => {
    it('carries the message it is given', () => {
        expect(new EslintInvalidParserOptionsException('a reason').message).toBe('a reason');
    });

    it('names itself, so a stack trace reads as this exception', () => {
        expect(new EslintInvalidParserOptionsException('a reason').name).toBe('EslintInvalidParserOptionsException');
    });

    it('extends the base for a bad argument', () => {
        expect(new EslintInvalidParserOptionsException('a reason')).toBeInstanceOf(EslintInvalidArgumentException);
    });

    it('extends the native error, so any caller can catch it', () => {
        expect(new EslintInvalidParserOptionsException('a reason')).toBeInstanceOf(Error);
    });
});
